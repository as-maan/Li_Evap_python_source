...
"""
def calc_OBS_para(shells,rukawat,src,elem,elem_r,elem_noruk,elem_noruk_t,thetas,n,i,nprocs):
	if i+1 == nprocs:
		for ii in range(i*n,(i+1)*n):
			...
			#print 'shell normal'
			N_p = get.normal(shells.normals[ii])
			#print N_p
			...
			if ray_angle < 90:  
				...
				for j in range(0,1):
					...
					#print 'obstruction element points'
					#print rwat1
					#print rwat2
					#print rwat3
					...
					N_rwat = -get.normal(rukawat.normals[j])
					print 'rwat Normal', N_rwat
			                # Find Q, the point along ray that lies on the plane of rwat
					#p_Q,t_Q = get.rwatQ(N_rwat,p_s,d,t,c_rwat)
					p_Q,t_Q = get.rwatQ(N_rwat,p_s,d,t,rwat1)
					print 'p_Q and t_q'
					print p_Q
					print t_Q
			    ...
			    #            print('now checking against all shell elements', i)   
					for k in [u for u in range(0,2) if u!=ii]:
						rwat1,rwat2,rwat3 = get.p(shells.x[k,:],shells.y[k,:],shells.z[k,:])
						#c_rwat = get.center(rwat1,rwat2,rwat3)
						N_rwat = -get.normal(shells.normals[k])
						print 'shell rwat Normal', N_rwat
						
						#p_Q,t_Q = get.rwatQ(N_rwat,p_s,d,t,c_rwat)
						p_Q,t_Q = get.rwatQ(N_rwat,p_s,d,t,rwat1)
						print 'p_Q and t_q from shell'
						print p_Q
						print t_Q	
						OBS = get.obstruction(rwat1,rwat2,rwat3,p_Q,t_Q,t,N_rwat,d)            
						if OBS == True:
		...
		for ii in range(i*n,(i+1)*n):
			...
			if ray_angle < 90:  
				...
				if OBS == False:
			    #            print('now checking against all shell elements', i)   
					for k in [u for u in range(0,1) if u!=ii]:
						rwat1,rwat2,rwat3 = get.p(shells.x[k,:],shells.y[k,:],shells.z[k,:])
						#c_rwat = get.center(rwat1,rwat2,rwat3)
						N_rwat = -get.normal(shells.normals[k])
						#p_Q,t_Q = get.rwatQ(N_rwat,p_s,d,t,c_rwat)
...
shells = mesh.Mesh.from_file('/media/amaan/Storage/Work/PPPL/Analysis/Li_Evap/shells/Li_Evap_source/CAD_Source/shell_Coarse-5872.stl')
rukawat = mesh.Mesh.from_file('/media/amaan/Storage/Work/PPPL/Analysis/Li_Evap/shells/Li_Evap_source/CAD_Source/Li_Evap_Export_Obs.STL')
r_qcm = 607.8082
t_qcm = 100.0
...
OP = calc_OBS_para(shells,rukawat,src,np.size(shells.areas),elem_r,OUTPUT['i'],OUTPUT['t'],OUTPUT['thetas'],int(n),rank,nprocs)
